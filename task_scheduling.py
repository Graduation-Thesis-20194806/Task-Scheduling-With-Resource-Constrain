...
def hill(isNormalHC):
    ...
    while current_heuristic[0] != 0:
        ...
        current_heuristic = heuristic(tasks)
        change += 1
        if (current_heuristic[0] < minimum_h or (
                current_heuristic[0] == minimum_h and current_heuristic[1] < minimum_c)):
            minimum_h = current_heuristic[0]
            minimum_c = current_heuristic[1]
            count = 0
...
def print_solution():
    ...
    for task in tasks:
        print(
            f'{bcolors.OKGREEN} Task {task.id} --> Machine {task.assigned_machine} {bcolors.ENDC}, {bcolors.OKCYAN} Runtime: {round(task.run_time, 2)} {bcolors.ENDC}, {bcolors.OKBLUE} Finish time: {round(task.finish_time, 2)} ms {bcolors.ENDC}, {bcolors.FAIL} Deadline: {round(task.deadline, 2)} ms {bcolors.ENDC}, Final cost: {round(task.final_cost, 2)}')
    ...
    if count == maximum_check:
        ...
        print()
    print(
        f'Heuristic:  {res[0]} Tasks reached deadline, Performance:{bcolors.OKCYAN} {round((num_tasks - res[0]) / num_tasks * 100, 2)}% {bcolors.ENDC}, total cost: {round(res[1], 2)}')
    print()
    k = 0
    cc = 0
    for node in nodes:
    ...
    while True:
        ...
        if choice == '1':
            ...
            for task in tasks:
                print(
                    f'{bcolors.OKCYAN}Task {task.id} {bcolors.ENDC}: {bcolors.OKGREEN} {task.amount} MI, Deadline: {task.deadline}ms {bcolors.ENDC}')
            for node in nodes:
                print(
                    f'{bcolors.HEADER}Node {node.id} {bcolors.ENDC}: {bcolors.OKBLUE} {node.speed} MIPS, Cost: {round(node.cost, 2)} G$/S {bcolors.ENDC} ')
...
def exportToXlsx():
    try:
        ...
        workbook.close()
        print(
            f'{bcolors.OKBLUE} Excel file (Result.xlsx) has been ctreated successfuly. it has been saved in the relative path')
    except:
        print(
            f"{bcolors.FAIL}Couldn't save the file because another program is using it or it's in read-only mode{bcolors.ENDC}")
...
def menu():
    ...
    print(f'----------------------------------------------\n {bcolors.ENDC}')
    print(
        f'{bcolors.OKGREEN}Before assigning the tasks to machines, the algorithm must be chosen in order to machines start the procces\n')
